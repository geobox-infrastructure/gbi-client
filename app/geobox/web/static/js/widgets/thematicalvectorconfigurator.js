var thematicalVectorConfiguratorLabel = {
    ...
    'maxColorsReached': OpenLayers.i18n('Max autogenerated colors reached')
};
...
gbi.widgets.ThematicalVectorConfigurator.prototype = {
    ...
    render: function() {
        ...
        $('#attribute').change(function() {
            if(self.mode == 'exact') {
                self.populateExactInputs($(this).val());
            } else {
                $('.exactInputControl tbody tr:not(.no-input)').remove();
            }
        });
    ...
    populateExactInputs: function(attribute) {
        var self = this;
        var attributeValues = self.activeLayer ? self.activeLayer.attributeValues(attribute) : [];
        var maxValues = 20;
        $('#max-auto-values-reached').addClass('hide');
        if(attributeValues.length > maxValues) {
            $('#max-auto-values-reached').removeClass('hide');
        }
        $('.exactInputControl tbody tr:not(.no-input)').remove()
        $.each(attributeValues.sort(), function(idx, value) {
            var element = self.addInput('exact', {'value': value});
    ...
    toggleExact: function(noPopulate) {
        var self = this;
        $('#max-auto-values-reached').addClass('hide');
        if(noPopulate !== true) {
            self.populateExactInputs($('#attribute').val())
        }
    ...
    addInput: function(mode, filterOption) {
        ...
        $('.exactInputControl tbody tr.no-input').first().addClass('hide');

        switch(mode) {
            case 'exact':
                idx = $('.exactInputControl tbody tr:not(.no-input)').length;
                ...
                break;
            case 'range':
                idx = $('.rangeInputControl tbody tr:not(.no-input)').length;
    ...
    execute: function() {
        ...
        switch(this.mode) {
            case 'exact':
                $('.exactInputControl tbody tr:not(.no-input)').each(function(idx, element) {
                    element = $(element);
                ...
                break;
            case 'range':
                $('.rangeInputControl tbody tr:not(.no-input)').each(function(idx, element) {
                    element = $(element);
...
gbi.widgets.ThematicalVectorConfigurator.template = '\
    ...
    <div class="hide pull-right alert alert-warning" id="max-auto-values-reached">' + thematicalVectorConfiguratorLabel.maxColorsReached + '</div>\
